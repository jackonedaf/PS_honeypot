...
void *http_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (client_sock >= 0)
        {
            ...
            log_connection(ip_str, ntohs(client_addr.sin_port), "HTTP");
...
void *ssh_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (client_sock >= 0)
        {
            char ip_str[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &client_addr.sin_addr, ip_str, sizeof(ip_str));
            log_connection(ip_str, ntohs(client_addr.sin_port), "SSH");
...
void *telnet_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (client_sock >= 0)
        {
            char ip_str[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &client_addr.sin_addr, ip_str, sizeof(ip_str));
            log_connection(ip_str, ntohs(client_addr.sin_port), "Telnet");
...
void start_multithreaded_honeypot()
{
